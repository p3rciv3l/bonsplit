...
struct PaneTilingTests {
    ...
    @Test
    func paneHostingCachePreservesNativeHostsAcrossEveryLayoutRestructure() throws {
        let controller = try makeController(paneCount: 4)
        let cache = PaneHostingCoordinator()
        let revision = UUID()
        var factoryCalls = 0
        let panes = controller.allPaneIds
        let hosts = Dictionary(uniqueKeysWithValues: panes.map { pane in
            let host = cache.host(for: pane, contentRevision: revision) {
                factoryCalls += 1
                return AnyView(EmptyView())
            }
            return (pane, host)
        })
        let nativeViews = hosts.mapValues { $0.view }
        let slots = panes.map { _ in NSView(frame: NSRect(x: 0, y: 0, width: 600, height: 400)) }
        for (index, pane) in panes.enumerated() {
            cache.attach(try #require(hosts[pane]), to: slots[index])
        }

        for action in [PaneTilingAction.tile, .moveNext, .promote, .increaseMasterCount, .monocle, .focusNext, .tile, .decreaseMasterCount, .manual] {
            #expect(controller.performTilingAction(action))
            for (index, pane) in controller.allPaneIds.enumerated() {
                let current = cache.host(for: pane, contentRevision: revision) {
                    factoryCalls += 1
                    return AnyView(EmptyView())
                }
                #expect(current === hosts[pane])
                #expect(current.view === nativeViews[pane])
                cache.attach(current, to: slots[index])
                #expect(current.view.superview === slots[index])
                #expect(slots[index].subviews.count == 1)
            }
        }
        #expect(factoryCalls == panes.count)
    }

    @Test
    func paneHostingContentRevisionRefreshesRootWithoutReplacingNativeHost() {
        let cache = PaneHostingCoordinator()
        let pane = PaneID()
        let firstRevision = UUID()
        var factoryCalls = 0
        let initial = cache.host(for: pane, contentRevision: firstRevision) {
            factoryCalls += 1
            return AnyView(Color.red)
        }
        let nativeView = initial.view
        let unchanged = cache.host(for: pane, contentRevision: firstRevision) {
            factoryCalls += 1
        ...
        let refreshed = cache.host(for: pane, contentRevision: secondRevision) {
            factoryCalls += 1
            return AnyView(Color.blue)
        }
        #expect(refreshed === initial)
        #expect(refreshed.view === nativeView)
        #expect(factoryCalls == 2)
        _ = cache.host(for: pane, contentRevision: secondRevision) {
            factoryCalls += 1
    ...
    @Test
    func paneHostingPrunesClosedEntriesWhileRetainingSurvivingHosts() {
        let cache = PaneHostingCoordinator()
        let livePane = PaneID()
        let closedPane = PaneID()
        let revision = UUID()
        let liveHost = cache.host(for: livePane, contentRevision: revision) { AnyView(EmptyView()) }
        let closedHost = cache.host(for: closedPane, contentRevision: revision) { AnyView(EmptyView()) }

        cache.retainPanes([livePane])

        let retained = cache.host(for: livePane, contentRevision: revision) { AnyView(EmptyView()) }
        let reopened = cache.host(for: closedPane, contentRevision: revision) { AnyView(EmptyView()) }
        #expect(retained === liveHost)
        #expect(reopened !== closedHost)
        #expect(reopened.view !== closedHost.view)
    }

    @Test
    func paneHostingRefreshesPresentationChangesWithinTheSameContentRevision() {
        let cache = PaneHostingCoordinator()
        let pane = PaneID()
        let revision = UUID()
        var factoryCalls = 0
        let initial = cache.host(for: pane, contentRevision: revision) {
            factoryCalls += 1
            return AnyView(EmptyView())
        }
        let nativeView = initial.view
        let options: [(Bool, TabBarVisibility, ContentViewLifecycle)] = [
            (false, .always, .recreateOnSwitch),
            (false, .multipleTabs, .recreateOnSwitch),
            (false, .multipleTabs, .keepAllAlive),
            (true, .multipleTabs, .keepAllAlive),
            (true, .always, .keepAllAlive),
            (true, .always, .recreateOnSwitch),
        ]

        for (index, option) in options.enumerated() {
            for _ in 0..<2 {
                let current = cache.host(
                    for: pane,
                    contentRevision: revision,
                    showSplitButtons: option.0,
                    tabBarVisibility: option.1,
                    contentViewLifecycle: option.2
                ) {
                    factoryCalls += 1
                    return AnyView(EmptyView())
                }
                #expect(current === initial)
                #expect(current.view === nativeView)
                #expect(factoryCalls == index + 2)
            }
        }
    }

    @Test
    func paneHostingReattachesIncomingDescendantBeforeDiscardingOldSubtree() {
        let cache = PaneHostingCoordinator()
        let host = cache.host(for: PaneID(), contentRevision: UUID()) { AnyView(EmptyView()) }
        let rootSlot = NSView(frame: NSRect(x: 0, y: 0, width: 700, height: 500))
        let obsoleteSubtree = NSView(frame: rootSlot.bounds)
        let innerSlot = NSView(frame: obsoleteSubtree.bounds)
        rootSlot.addSubview(obsoleteSubtree)
        obsoleteSubtree.addSubview(innerSlot)
        cache.attach(host, to: innerSlot)
        let nativeView = host.view

        cache.attach(host, to: rootSlot)

        #expect(host.view === nativeView)
        #expect(nativeView.superview === rootSlot)
        #expect(rootSlot.subviews.count == 1)
        #expect(obsoleteSubtree.superview == nil)
        #expect(innerSlot.subviews.isEmpty)
        #expect(nativeView.frame == rootSlot.bounds)
        cache.attach(host, to: rootSlot)
        #expect(rootSlot.subviews.count == 1)
        #expect(rootSlot.subviews[0] === nativeView)
    }

    @Test
    func paneHostingCacheAndNativeHostReleaseAfterOwnerIsDiscarded() {
        weak var weakCache: PaneHostingCoordinator?
        weak var weakHost: NonDraggableHostingController<AnyView>?
        autoreleasepool {
            let cache = PaneHostingCoordinator()
            let host = cache.host(for: PaneID(), contentRevision: UUID()) { AnyView(EmptyView()) }
            let slot = NSView(frame: NSRect(x: 0, y: 0, width: 500, height: 300))
    ...
    func nativeLeafAnchorSurvivesSinglePaneSplitAndPromotionWithNewContentRevisions(refreshContentRevision: Bool) throws {
        _ = NSApplication.shared
        ...
        let originalPane = try #require(controller.focusedPaneId)
        let originalTab = try #require(controller.selectedTab(inPane: originalPane))
        ...
        let originalAnchor = try #require(findNativeAnchor(originalTab.id, in: hostingView))
        let originalHost = try #require(findNativeAnchorHost(for: originalAnchor))

        hostingView.rootView = anchorTestView(controller: controller)
        hostingView.layoutSubtreeIfNeeded()
        #expect(findNativeAnchor(originalTab.id, in: hostingView) === originalAnchor)
        #expect(findNativeAnchor(originalTab.id, in: hostingView).flatMap { findNativeAnchorHost(for: $0) } === originalHost)

        let newPane = try #require(controller.splitPane(originalPane, orientation: .horizontal, withTab: Tab(title: "Second")))
        if refreshContentRevision {
            hostingView.rootView = anchorTestView(controller: controller)
        }
        hostingView.layoutSubtreeIfNeeded()
        #expect(findNativeAnchor(originalTab.id, in: hostingView) === originalAnchor)
        #expect(findNativeAnchor(originalTab.id, in: hostingView).flatMap { findNativeAnchorHost(for: $0) } === originalHost)
        #expect(originalHost.window === window)
        let newTab = try #require(controller.selectedTab(inPane: newPane))
        let newAnchor = try #require(findNativeAnchor(newTab.id, in: hostingView))
        let newHost = try #require(findNativeAnchorHost(for: newAnchor))

        #expect(controller.closePane(newPane))
        if refreshContentRevision {
            hostingView.rootView = anchorTestView(controller: controller)
        }
        hostingView.layoutSubtreeIfNeeded()
        ...
        #expect(findNativeAnchor(originalTab.id, in: hostingView).flatMap { findNativeAnchorHost(for: $0) } === originalHost)
        #expect(originalHost.window === window)
        #expect(findNativeAnchor(newTab.id, in: hostingView) == nil)
        #expect(newHost.window == nil)
    }

    @Test
    func nonanimatedTilingRetainsEveryNativeLeafAcrossLayoutReorderAndZoom() throws {
        _ = NSApplication.shared
        let controller = try makeController(paneCount: 4)
        controller.configuration.appearance.enableAnimations = false
        controller.configuration.contentViewLifecycle = .keepAllAlive
        let hostingView = NSHostingView(rootView: anchorTestView(controller: controller))
        let window = NSWindow(
        ...
        window.contentView = hostingView
        hostingView.layoutSubtreeIfNeeded()
        var anchors: [TabID: NSView] = [:]
        var hosts: [PaneID: NSView] = [:]
        for pane in controller.allPaneIds {
            for tab in controller.tabs(inPane: pane) {
                let anchor = try #require(findNativeAnchor(tab.id, in: hostingView))
                anchors[tab.id] = anchor
                hosts[pane] = try #require(findNativeAnchorHost(for: anchor))
            }
        }

        for action in [PaneTilingAction.tile, .moveNext, .movePrevious, .increaseMasterCount,
                       .decreaseMasterCount, .increaseMasterRatio, .decreaseMasterRatio, .promote,
                       .monocle, .focusNext, .focusPrevious, .tile, .manual] {
            #expect(controller.performTilingAction(action))
            hostingView.rootView = anchorTestView(controller: controller)
            hostingView.layoutSubtreeIfNeeded()
            for pane in controller.allPaneIds {
                ...
                for tab in controller.tabs(inPane: pane) {
                    let anchor = try #require(findNativeAnchor(tab.id, in: hostingView))
                    #expect(anchor === anchors[tab.id], "Native leaf changed during \(action.rawValue)")
            ...
            if controller.tilingLayout == .monocle {
                ...
                let focusedHost = try #require(hosts[focused])
                let actual = focusedHost.convert(focusedHost.bounds, to: hostingView)
                #expect(abs(actual.width - hostingView.bounds.width) <= 1)
                #expect(abs(actual.height - hostingView.bounds.height) <= 1)
                for split in nativeSplits(in: hostingView) where !split.isHiddenOrHasHiddenAncestor {
    ...
    func nonanimatedRendererRetainsLayoutAndNativeContentAcrossInteractivityChanges(layout: PaneTilingAction) throws {
        ...
        window.contentView = hostingView
        hostingView.layoutSubtreeIfNeeded()
        let nativeRoot = try #require(findNativeTreeRoot(in: hostingView))
        ...
        for interactive in [false, true] {
            ...
            hostingView.layoutSubtreeIfNeeded()
            #expect(findNativeTreeRoot(in: hostingView) === nativeRoot)
    ...
    private func findNativeTreeRoot(in view: NSView) -> NativeSplitTreeContainer? {
        if let root = view as? NativeSplitTreeContainer { return root }
        for child in view.subviews {
            if let root = findNativeTreeRoot(in: child) { return root }
        }
        return nil
    }

    @Test
    func cachedDirectNativeLeafSurvivesReparentingAndContentRevision() throws {
        _ = NSApplication.shared
        let cache = PaneHostingCoordinator()
        let pane = PaneID()
        let tab = TabID()
        let revision = UUID()
        let host = cache.host(for: pane, contentRevision: revision) { AnyView(NativeAnchorProbe(tabID: tab)) }
        let window = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: 900, height: 600),
            styleMask: .borderless,
            backing: .buffered,
        ...
        defer { window.orderOut(nil) }
        let root = try #require(window.contentView)
        let firstSlot = NSView(frame: NSRect(x: 0, y: 0, width: 450, height: 600))
        let secondSlot = NSView(frame: NSRect(x: 450, y: 0, width: 450, height: 600))
        root.addSubview(firstSlot)
        root.addSubview(secondSlot)
        cache.attach(host, to: firstSlot)
        root.layoutSubtreeIfNeeded()
        let original = try #require(findNativeAnchor(tab, in: host.view))

        cache.attach(host, to: secondSlot)
        root.layoutSubtreeIfNeeded()
        #expect(findNativeAnchor(tab, in: host.view) === original)
        let refreshed = cache.host(for: pane, contentRevision: UUID()) { AnyView(NativeAnchorProbe(tabID: tab)) }
        root.layoutSubtreeIfNeeded()
        #expect(refreshed === host)
        #expect(findNativeAnchor(tab, in: host.view) === original)
    }

    @Test
    func independentSwiftUIParentGraphReplacementCharacterizesLegacyLeafRemount() throws {
        _ = NSApplication.shared
        let cache = PaneHostingCoordinator()
        let tab = TabID()
        let host = cache.host(for: PaneID(), contentRevision: UUID()) { AnyView(NativeAnchorProbe(tabID: tab)) }
        let window = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: 900, height: 600),
            styleMask: .borderless,
            backing: .buffered,
        ...
        defer { window.orderOut(nil) }
        let root = try #require(window.contentView)
        let parking = NSView(frame: root.bounds)
        root.addSubview(parking)
        cache.registerParkingView(parking)
        let firstGraph = NSHostingView(rootView: AnyView(CachedPaneSlotProbe(cache: cache, host: host)))
        let secondGraph = NSHostingView(rootView: AnyView(EmptyView()))
        firstGraph.frame = NSRect(x: 0, y: 0, width: 450, height: 600)
        secondGraph.frame = NSRect(x: 450, y: 0, width: 450, height: 600)
        root.addSubview(firstGraph)
        root.addSubview(secondGraph)
        root.layoutSubtreeIfNeeded()
        let original = try #require(findNativeAnchor(tab, in: host.view))
        let nativeHost = host.view

        firstGraph.rootView = AnyView(EmptyView())
        firstGraph.layoutSubtreeIfNeeded()
        #expect(host.view === nativeHost)
        #expect(host.view.window === window)
        #expect(host.view.superview === parking)
        #expect(findNativeAnchor(tab, in: host.view) === original)

        secondGraph.rootView = AnyView(CachedPaneSlotProbe(cache: cache, host: host))
        secondGraph.layoutSubtreeIfNeeded()
        #expect(host.view === nativeHost)
        #expect(host.view.window === window)
        #expect(host.view.isDescendant(of: secondGraph))
        let reattachedAnchor = try #require(findNativeAnchor(tab, in: host.view))
        // This isolates the legacy animated renderer's cross-graph limitation.
        // The nonanimated Bonsplit renderer must satisfy the unqualified native
        // continuity tests above instead of depending on this unsupported move.
        withKnownIssue("SwiftUI may remount a cached host's child when attaching it to a different parent graph", isIntermittent: true) {
            #expect(reattachedAnchor === original)
        }
    }

    @MainActor
    private struct CachedPaneSlotProbe: NSViewRepresentable {
        let cache: PaneHostingCoordinator
        let host: NonDraggableHostingController<AnyView>

        func makeCoordinator() -> PaneHostingCoordinator { cache }

        func makeNSView(context: Context) -> PaneHostingSlotView {
            let slot = PaneHostingSlotView()
            cache.attach(host, to: slot)
            return slot
        }

        func updateNSView(_ nsView: PaneHostingSlotView, context: Context) {
            cache.attach(host, to: nsView)
        }

        static func dismantleNSView(_ nsView: PaneHostingSlotView, coordinator: PaneHostingCoordinator) {
            coordinator.parkHostedPanes(in: nsView)
        }
    }
    ...
    @Test
    func parkingPreservesLeafDuringSourceRemovalAndDeferredDestinationAttachment() throws {
        let fixture = try makeParkingFixture()
        defer { fixture.window.orderOut(nil) }
        let root = try #require(fixture.window.contentView)
        let original = try #require(findNativeAnchor(fixture.tab, in: fixture.host.view))
        let originalFrame = fixture.host.view.convert(fixture.host.view.bounds, to: nil)

        fixture.slot.removeFromSuperview()
        #expect(fixture.host.view.superview === fixture.parking)
        #expect(fixture.host.view.window === fixture.window)
        #expect(fixture.host.view.convert(fixture.host.view.bounds, to: nil) == originalFrame)
        #expect(findNativeAnchor(fixture.tab, in: fixture.host.view) === original)

        let destination = PaneHostingSlotView(frame: NSRect(x: 300, y: 30, width: 400, height: 500))
        fixture.cache.attach(fixture.host, to: destination)
        #expect(fixture.host.view.superview === fixture.parking)
        root.addSubview(destination)
        root.layoutSubtreeIfNeeded()
        #expect(fixture.host.view.superview === destination)
        #expect(fixture.host.view.window === fixture.window)
        #expect(fixture.host.view.frame == destination.bounds)
        #expect(findNativeAnchor(fixture.tab, in: fixture.host.view) === original)
    }

    @Test
    func coordinatorKeepsMountedChildKindsUntilNativeHostsAreReplaced() {
        let leaf = PaneState()
        let branch = SplitState(
            orientation: .vertical,
            first: .pane(PaneState()),
            second: .pane(PaneState())
        )
        let mounted = SplitState(orientation: .horizontal, first: .pane(leaf), second: .split(branch))
        let coordinator = SplitContainerView<EmptyView, EmptyView>.Coordinator(
            splitState: mounted,
            minimumPaneWidth: 100,
            minimumPaneHeight: 100,
        ...
        let cachedLeaf = NonDraggableHostingController(rootView: AnyView(EmptyView()))
        coordinator.firstHostingController = cachedLeaf
        let replacement = SplitState(orientation: .horizontal, first: .split(branch), second: .pane(leaf))

        coordinator.update(
            splitState: replacement,
            minimumPaneWidth: 100,
            minimumPaneHeight: 100,
            onGeometryChange: nil
        )

        #expect(coordinator.splitState === replacement)
        #expect(coordinator.firstHostingController === cachedLeaf)
        #expect(coordinator.firstNodeType == .pane)
        #expect(coordinator.secondNodeType == .split)
        #expect(coordinator.firstNodeType != replacement.first.nodeType)
        #expect(coordinator.secondNodeType != replacement.second.nodeType)
    }

    @Test
    func staleDeferredSlotCannotReclaimHostFromNewerDestination() throws {
        let fixture = try makeParkingFixture()
        defer { fixture.window.orderOut(nil) }
        let root = try #require(fixture.window.contentView)
        let stale = PaneHostingSlotView(frame: fixture.slot.frame)
        let destination = PaneHostingSlotView(frame: fixture.slot.frame)
        fixture.cache.attach(fixture.host, to: stale)
        fixture.cache.attach(fixture.host, to: destination)

        root.addSubview(destination)
        #expect(fixture.host.view.superview === destination)
        root.addSubview(stale)
        #expect(fixture.host.view.superview === destination)
        #expect(stale.subviews.isEmpty)
    }
    ...
    @Test
    func inheritedHiddenStateIsPreservedInParkingAndClearedByVisibleDestination() throws {
        let fixture = try makeParkingFixture()
        defer { fixture.window.orderOut(nil) }
        let root = try #require(fixture.window.contentView)
        fixture.slot.isHidden = true
        #expect(!fixture.host.view.isHidden)
        #expect(fixture.host.view.isHiddenOrHasHiddenAncestor)
    ...
    @Test
    func pruningBeforeOldSlotRemovalDoesNotParkAClosedHost() throws {
        let fixture = try makeParkingFixture()
        defer { fixture.window.orderOut(nil) }
        fixture.cache.retainPanes([])
        fixture.slot.removeFromSuperview()
        #expect(fixture.host.view.window == nil)
        #expect(fixture.host.view.superview === fixture.slot)
        #expect(fixture.parking.subviews.isEmpty)
    }

    private func makeParkingFixture() throws -> (
        window: NSWindow,
        cache: PaneHostingCoordinator,
        parking: NSView,
        slot: PaneHostingSlotView,
        host: NonDraggableHostingController<AnyView>,
        tab: TabID
    ) {
        _ = NSApplication.shared
        let window = NSWindow(
        ...
        let root = try #require(window.contentView)
        let parking = NSView(frame: root.bounds)
        let slot = PaneHostingSlotView(frame: NSRect(x: 40, y: 20, width: 300, height: 400))
        root.addSubview(parking)
        ...
        cache.attach(host, to: slot)
        root.layoutSubtreeIfNeeded()
        return (window, cache, parking, slot, host, tab)
    }
